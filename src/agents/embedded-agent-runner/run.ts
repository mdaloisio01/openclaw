...
import { recordGovernedMissionWithheldPayload } from "../../governance/governed-mission-runtime.js";
...
function withGovernedMissionOutputWithheld(
  result: EmbeddedAgentRunResult,
  governedOutputWithheld: boolean,
): EmbeddedAgentRunResult {
  if (!governedOutputWithheld) {
    return result;
  }
  ...
  return {
    ...
    meta: {
      ...
      error: result.meta.error
        ? { ...result.meta.error, message: GOVERNED_FINAL_RELEASE_WITHHELD_NOTICE }
        : undefined,
...
export async function runEmbeddedAgent(
  ...
  // Reserve the owner before queueing or classifying the turn. Admission must not
  // convert a session while an already-started ordinary turn can still execute.
  const releaseGovernedMissionAdmissionFence =
    beginOwnerRunForGovernedMissionAdmission(governedMissionOwnerKey);
  const runPromise = Promise.resolve().then(() =>
    enqueueSession(() => {
      ...
      return enqueueGlobal(async () => {
        ...
        let governedMissionContentHooksSuppressed =
          hasGovernedMissionClaimForOwnerKey(governedMissionOwnerKey);
        // Once any attempt is governed, every later loop exit must stay withheld.
        // Resetting this per attempt can publish retry-limit failures before release.
        let governedMissionOutputWithheld = governedMissionContentHooksSuppressed;
        const refreshGovernedMissionContentSuppression = (): boolean => {
          governedMissionContentHooksSuppressed =
            governedMissionContentHooksSuppressed ||
            hasGovernedMissionClaimForOwnerKey(governedMissionOwnerKey);
          return governedMissionContentHooksSuppressed;
        };
        ...
        if (
          !refreshGovernedMissionContentSuppression() &&
          params.trigger === "cron" &&
        ...
        const hookSelection = await resolveHookModelSelection({
          ...
          provider,
          modelId,
          hookRunner: refreshGovernedMissionContentSuppression() ? undefined : hookRunner,
          hookContext: hookCtx,
        });
        ...
        if (refreshGovernedMissionContentSuppression() && pluginHarnessOwnsTransport) {
          throw new Error(
            `Governed mission execution requires the OpenClaw harness; selected harness ${agentHarness.id} cannot enforce the durable release boundary.`,
          );
        }
        ...
        const pluginHarnessForwardedProfileCandidates = pluginHarnessOwnsTransport
          ...
          : [];
        const profileFailureStore = pluginHarnessOwnsTransport
        ...
        let pendingGovernedLeaseClose: { flowId: string; runId: string } | undefined;
        const closePendingGovernedLease = async () => {
          const lease = pendingGovernedLeaseClose;
          if (lease) {
            await closeGovernedExecutionLeaseWithRetry({
              close: () => closeGovernedMissionExecutionLease(lease),
            ...
            if (pendingGovernedLeaseClose === lease) {
              pendingGovernedLeaseClose = undefined;
            }
          }
        };
        try {
          ...
          while (true) {
            // A retry cannot open another lease while the prior attempt still owns it.
            await closePendingGovernedLease();
            if (runLoopIterations >= MAX_RUN_LOOP_ITERATIONS) {
              ...
              return withGovernedMissionOutputWithheld(
            ...
            const governedMissionSuppressedAtDispatch = refreshGovernedMissionContentSuppression();
            governedMissionOutputWithheld ||= governedMissionSuppressedAtDispatch;
            if (governedMissionSuppressedAtDispatch && pluginHarnessOwnsTransport) {
              throw new Error(
                `Governed mission execution requires the OpenClaw harness; selected harness ${agentHarness.id} cannot enforce the durable release boundary.`,
              );
            }
            const rawAttempt = await runEmbeddedAttemptWithBackend({
              ...
              governedMissionContentHooksSuppressed: governedMissionSuppressedAtDispatch,
            ...
            if (rawAttempt.governedMissionFlowId && rawAttempt.governedMissionExecutionRunId) {
              pendingGovernedLeaseClose = {
                flowId: rawAttempt.governedMissionFlowId,
                runId: rawAttempt.governedMissionExecutionRunId,
              };
            }
            ...
            governedMissionOutputWithheld ||=
              governedMissionContentHooksSuppressed || Boolean(attempt.governedMissionFlowId);
            const governedOutputWithheld = governedMissionOutputWithheld;
            const publishedSystemPromptReport = governedOutputWithheld
              ? undefined
              : attempt.systemPromptReport;
            const publishedFinalPromptText = governedOutputWithheld
              ? undefined
            ...
            if (contextOverflowError) {
              ...
              if (!governedOutputWithheld && !toolResultTruncationAttempted) {
            ...
            // The governed attempt result is an internal holding area until release.
            // Downstream metadata consumers are output channels too, so redact content here.
            const publishedFinalAssistantVisibleText = governedOutputWithheld
              ? undefined
              : finalAssistantVisibleText;
            const publishedFinalAssistantRawText = governedOutputWithheld
              ? undefined
            ...
            const completePayloads = mergeAttemptToolMediaPayloads({
              payloads,
            ...
            const payloadsWithToolMedia: ReplyPayload[] | undefined = governedOutputWithheld
              ? [
                  {
                    text: GOVERNED_FINAL_RELEASE_WITHHELD_NOTICE,
                    isStatusNotice: true,
              ...
              return withGovernedMissionOutputWithheld(
                {
                  ...
                  meta: {
                    ...
                    systemPromptReport: publishedSystemPromptReport,
                    finalPromptText: publishedFinalPromptText,
                    finalAssistantVisibleText: publishedFinalAssistantVisibleText,
                    finalAssistantRawText: publishedFinalAssistantRawText,
                    replayInvalid,
                    livenessState,
                    timeoutPhase,
                    providerStarted,
            ...
            const payloadsForTerminalPath = recoveredFinalAssistantPayloadsAfterPromptTimeout
              ...
              : payloadsWithToolMedia?.length
                ...
                : silentToolResultReplyPayload
                  ? [silentToolResultReplyPayload]
                  : payloadsWithToolMedia;
            // The withholding notice is a public status wrapper, not model output.
            // Classify completion from the accepted private payload so an empty
            // governed turn can still become the canonical silent payload.
            const payloadCount = governedOutputWithheld
              ? (completePayloads?.length ?? 0)
              : (payloadsForTerminalPath?.length ?? 0);
            ...
            if (
              ...
              if (planDetails && !governedOutputWithheld) {
                emitAgentPlanEvent({
                  runId: params.runId,
                  ...(params.sessionKey ? { sessionKey: params.sessionKey } : {}),
                  data: {
            ...
            const governedTerminalPayloads = emptyAssistantReplyIsSilent
              ? terminalPayloads
              : completePayloads;
            attempt.setTerminalLifecycleMeta?.({
              replayInvalid,
            ...
            if (
              attempt.governedMissionFlowId &&
              attempt.governedMissionAttemptReceiptId &&
              governedTerminalPayloads?.length
            ) {
              // Only the accepted terminal path may define the releasable payload.
              // Retry, timeout, and incomplete-turn branches exit before this point.
              const captured = recordGovernedMissionWithheldPayload({
                flowId: attempt.governedMissionFlowId,
                attemptReceiptId: attempt.governedMissionAttemptReceiptId,
                payload: governedTerminalPayloads,
              });
              if (!captured.applied) {
                log.warn(
                  `governed final payload identity was not persisted: flowId=${attempt.governedMissionFlowId} reason=${captured.reason}`,
                );
              }
            }
            return withGovernedMissionOutputWithheld(
          ...
          // Keep the lease fenced through every asynchronous cleanup step.
          await closePendingGovernedLease();
        }
      });
    }),
  ...
  try {
    ...
    if (countActiveToolExecutions(params.runId) > 0) {
      // Timed-out tools can outlive the run promise. Keep admission fenced until
      // the last execution ends so ordinary work cannot cross into a governed run.
      void waitForActiveToolExecutionsToDrain(params.runId).then(
        releaseGovernedMissionAdmissionFence,
      );
    } else {
      releaseGovernedMissionAdmissionFence();
    }
